...
def main() -> None:
    ...
    print(
        ...
        "- Entries and averaging legs fill at the DAILY CLOSE, and the two arms are NOT\n"
        "  symmetric about that: 52w_low's signal uses the trigger day's own Low, so it is\n"
        "  known intraday and filling at that day's close is realistic. rsi_dip's signal uses\n"
        "  only data strictly BEFORE the trigger day, so it is known at the PREVIOUS close --\n"
        "  filling at the trigger day's close gives it a full extra day of drift the signal\n"
        "  never asked for. Entering at that day's OPEN would be the realistic execution for\n"
        "  rsi_dip. Which way this biases the result is not known; it is not negligible.\n"
